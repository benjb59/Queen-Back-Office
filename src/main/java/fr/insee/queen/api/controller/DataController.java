...
public class DataController {
	...
	*/
	...
	public ResponseEntity<Object> putData(@RequestBody JsonNode dataValue, @PathVariable(value = "id") String id, HttpServletRequest request) {
		String userId = utilsService.getUserId(request);
		if(!userId.equals("GUEST") && !utilsService.checkHabilitation(request, id, Constants.INTERVIEWER)) {
			LOGGER.error("PUT data for reporting unit with id {} resulting in 403", id);
			return new ResponseEntity<>(HttpStatus.FORBIDDEN);
		}
		dataService.updateDataWithoutHibernate(id, dataValue);
